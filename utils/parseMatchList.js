...
function parseMatchList(tekst, { domyslneBo = 3 } = {}) {
  ...
  linie.forEach((surowa, index) => {
    ...
    if (!separator) {
      bledy.push({
        ...
        tresc: surowa.trim(),
        powod:
          'Nie rozpoznano dwóch drużyn — użyj "A vs B", "A - B", "A | B" albo "A; B".',
      });
      return;
    }
  ...
  for (const m of mecze) {
    const klucz = [m.teamA.toLowerCase(), m.teamB.toLowerCase()]
      .sort()
      .join("|");
    ...
    if (widziane.has(klucz)) {
      duplikaty.push({
        linia: m.linia,
        tresc: `${m.teamA} vs ${m.teamB}`,
        pierwsza: widziane.get(klucz),
      });
    } else {
