...
async function syncProposals(pool, { guildId, event, provider }) {
    if (!event.external_tournament_id) {
        const err = new Error('Event nie ma przypisanego turnieju u dostawcy (external_tournament_id)');
    ...
    for (const d of wynik.dopasowane) {
        ...
        await pool.query(
            ...
            `,
            [
                guildId, event.id, d.match.id, provider.name,
                d.provider.externalId, d.resA, d.resB,
    ...
    return {
        ...
        zapisanych: zapisane,
        nierozpoznaneDruzyny: wynik.nierozpoznaneDruzyny.map((x) => x.nierozpoznane).flat(),
