    ...
    return withGuild(guildId, async ({ pool }) => {
      ...
      if (phase === "swiss" && !inputStage) {
        return interaction.reply({
          ephemeral: true,
          content:
            "❌ Dla fazy Swiss musisz podać etap, np. `1`, `2` albo `3`.",
        });
      }

      const deadlineDate = DateTime.fromFormat(rawInput, "yyyy-MM-dd HH:mm", {
        zone: "Europe/Warsaw",
      });
      ...
      if (phase === "swiss") {
        ...
        if (!stageNumber) {
          return interaction.reply({
            ephemeral: true,
            content:
              "❌ Niepoprawny etap Swiss. Użyj np. `1`, `2`, `3`, `stage1`.",
          });
        }
