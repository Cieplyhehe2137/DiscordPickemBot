...
export default function DoubleElimResultsPanel({ slug, guildId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            describeActionError(err, "wczytać wyniki Double Elimination"),
          );
      } finally {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(
        err?.status === 400
          ? err.message
          : describeActionError(err, "zapisać wyniki Double Elimination"),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
      ...
      <h2 className="mt-2 text-3xl font-black">Double Elimination</h2>
