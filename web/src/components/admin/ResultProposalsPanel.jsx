...
export default function ResultProposalsPanel({ slug, onResultApplied }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.error(err);
        if (!cancelled)
          setError(describeActionError(err, "wczytać propozycji wyników"));
      } finally {
  ...
  async function handleSync() {
    try {
      ...
      if (s.nierozpoznaneDruzyny?.length) {
        czesci.push(
          `nierozpoznane drużyny: ${[...new Set(s.nierozpoznaneDruzyny)].join(", ")}`,
        );
      }
  ...
  async function handleZatwierdz(p) {
    const zmiana =
      p.obecny_res_a !== null && p.obecny_res_a !== undefined
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 p-5">
      ...
      <p className="mt-1 text-sm text-white/50">
        ...
        każdy wynik wpisujesz świadomie, bo zatwierdzenie przelicza punkty
        graczy.
      </p>

      {!providerConfigured && (
        <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm font-bold text-amber-200">
          Dostawca wyników nie jest skonfigurowany — ustaw{" "}
          <code>RESULT_PROVIDER</code> w <code>server/.env</code>.
        </div>
      )}
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3">
        <label className="flex-1">
          ...
          <span className="mt-2 block text-xs text-white/40">
            Zwykle chcesz <code>serie:ID</code> — jeden turniej u Was to u
            dostawcy cała seria (np. IEM Cologne to Stage&nbsp;1-3 plus
            Playoffs).
