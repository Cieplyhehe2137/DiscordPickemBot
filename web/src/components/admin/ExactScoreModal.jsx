...
export default function ExactScoreModal({ match, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(describeActionError(err, "wczytać dokładne wyniki"));
      } finally {
  ...
  function updateMap(mapNo, field, value) {
    setMaps((prev) =>
      prev.map((m) => (m.mapNo === mapNo ? { ...m, [field]: value } : m)),
    );
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(
        err?.status === 400
          ? err.message
          : describeActionError(err, "zapisać dokładne wyniki"),
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-zinc-950 p-8 text-white shadow-2xl">
                ...
                <div className="mt-2 grid grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      ...
                      value={m.exactA}
                      onChange={(e) =>
                        updateMap(m.mapNo, "exactA", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={m.exactB}
                      onChange={(e) =>
                        updateMap(m.mapNo, "exactB", e.target.value)
                      }
                      className="mt-2 w-full rounded-2xl border border-white/10 bg-black/30 px-5 py-4 text-white outline-none focus:border-violet-400/50"
                    />
                  </div>
        ...
        <p className="mt-4 text-sm text-white/40">
          Wynik rundy dla każdej mapy (0-99). Zapis przelicza punkty dla
          wszystkich typów tego meczu.
        </p>
