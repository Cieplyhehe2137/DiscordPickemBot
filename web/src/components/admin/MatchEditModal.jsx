...
export default function MatchEditModal({
  ...
  const zmienione =
    teamA !== match.team_a ||
    teamB !== match.team_b ||
    Number(bestOf) !== Number(match.best_of);
  ...
  const cokolwiekZniknie =
    u && (u.typy || u.typyMap || u.wyniki || u.wynikiMap || u.punkty);
      ...
      <div className="max-h-[90vh] w-full max-w-xl overflow-auto rounded-[2rem] border border-white/10 bg-zinc-950 p-8">
        ...
        {zmienione && (
          <p className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm font-bold text-amber-200">
            Zmiana drużyn albo formatu unieważnia dotychczasowe punkty tego
            meczu — zostaną przeliczone od razu po zapisie. Typy graczy zostają
            nietknięte.
          </p>
        )}
            ...
            <>
              ...
              <button
                ...
                disabled={
                  busy ||
                  (cokolwiekZniknie &&
                    potwierdzenie.trim() !== oczekiwanePotwierdzenie)
                }
