  ...
  async function handleSave() {
    ...
    try {
      if (deadlineType === "pick") {
        await setDeadline(guildId, {
          phase,
          data,
          stage: phase === "swiss" ? stage : undefined,
        });
      } else {
      ...
      setError(
        err?.status === 400 || err?.status === 404
          ? err.message
          : describeActionError(err, "zapisać deadline"),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-zinc-950 p-8 text-white shadow-2xl">
        ...
        <h2 className="mt-2 text-3xl font-black">Ustaw deadline</h2>

        <p className="mt-2 text-sm text-white/40">
          Dotyczy aktualnie aktywnego panelu Discorda dla tej fazy. Czasy
          podawane są w strefie Europe/Warsaw.
        </p>
        ...
        <div className="mt-6 grid gap-5">
          ...
          {deadlineType === "match" && phase === "swiss" && (
            <p className="text-sm text-yellow-300/80">
              Deadline wyników meczów nie jest per-etapowy dla Swiss w obecnym
              bocie - to wierna replika zachowania Discorda, nie błąd tego
              panelu.
            </p>
          )}
