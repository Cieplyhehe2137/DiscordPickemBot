...
export default function SwissResultsPanel({ slug, guildId }) {
    ...
    const [selection, setSelection] = useState({ x3_0: [], x0_3: [], advancing: [] });
    ...
    function toggleTeam(categoryKey, team) {
        setSelection((prev) => {
            ...
            return {
                ...prev,
                [categoryKey]: has ? current.filter((t) => t !== team) : [...current, team]
            };
        });
    }
    ...
    const currentStageLabel = STAGES.find((s) => s.value === stage)?.label || stage;
            ...
            <div className="mt-6 flex flex-wrap gap-2">
                {STAGES.map((s) => (
                    <button
                        ...
                        className={`rounded-xl px-4 py-2 text-sm font-black transition ${stage === s.value
