...
export default function PlayInResultsPanel({ slug, guildId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(describeActionError(err, "wczytać wyniki Play-In"));
      } finally {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(
        err?.status === 400
          ? err.message
          : describeActionError(err, "zapisać wyniki Play-In"),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
      ...
      <h2 className="mt-2 text-3xl font-black">Play-In</h2>

      <p className="mt-2 text-sm text-white/40">
        Wybierz dokładnie {REQUIRED} awansujących drużyn ({selected.length}/
        {REQUIRED})
      </p>
