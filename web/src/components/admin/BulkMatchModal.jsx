...
export default function BulkMatchModal({ guildId, slug, onClose, onCreated }) {
    ...
    async function sprawdz() {
        try {
            ...
            const wynik = await bulkCreateMatches(guildId, slug, {
                phase, text, defaultBestOf: Number(defaultBestOf), dryRun: true
            });
    ...
    async function utworz() {
        try {
            ...
            const wynik = await bulkCreateMatches(guildId, slug, {
                phase, text, defaultBestOf: Number(defaultBestOf)
            });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-6">
            <div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-[2rem] border border-white/10 bg-zinc-950 p-8">
                ...
                <p className="mt-2 text-white/50">
                    Wklej listę par — jedna na linię. Nic nie powstanie, dopóki nie zobaczysz
                    podglądu i nie potwierdzisz.
                </p>

                <div className="mt-6 flex flex-wrap gap-4">
                    <label className="flex-1">
                        <span className="text-xs uppercase tracking-[0.2em] text-white/40">Faza</span>
                        ...
                        >
                            {FAZY.map((f) => <option key={f} value={f}>{f}</option>)}
                        </select>
                    </label>
                ...
                <label className="mt-5 block">
                    ...
                    <span className="mt-2 block text-xs text-white/40">
                        Rozdzielaj <code>vs</code>, <code>-</code>, <code>|</code> albo <code>;</code>.
                        Dopisz <code>BO1</code> na końcu linii, żeby nadpisać domyślne.
                        Puste linie i zaczynające się od <code>#</code> są pomijane.
                    </span>
                </label>
                ...
                {podglad && (
                    <div className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-5">
                        <p className="text-lg font-black">
                            Do utworzenia: <span className="text-violet-300">{doUtworzenia}</span>
                            {podglad.rozpoznanych !== doUtworzenia && (
                                <span className="text-white/40"> z {podglad.rozpoznanych} rozpoznanych</span>
                            )}
                        </p>
                        ...
                        {podglad.nieznaneDruzyny?.length > 0 && (
                            <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/10 p-4">
                                ...
                                {podglad.wskazowka && (
                                    <p className="mt-2 text-sm text-amber-200/70">{podglad.wskazowka}</p>
                                )}
                            </div>
                        )}
                        ...
                        {podglad.bledy?.length > 0 && (
                            <div className="mt-4">
                                ...
                                <ul className="mt-2 space-y-1 text-sm text-white/60">
                                    {podglad.bledy.map((b) => (
                                        <li key={b.linia}>
                                            <span className="font-mono text-white/40">linia {b.linia}:</span>{' '}
                                            „{b.tresc}” — {b.powod}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        {podglad.duplikaty?.length > 0 && (
                            <div className="mt-4">
                                <p className="text-sm font-black text-amber-300">
                                    Powtórzone pary ({podglad.duplikaty.length}) — zostaną utworzone,
                                    bo rewanż jest dozwolony:
                                </p>

                                <ul className="mt-2 space-y-1 text-sm text-white/60">
                                    {podglad.duplikaty.map((d) => (
                                        <li key={d.linia}>
                                            <span className="font-mono text-white/40">linia {d.linia}:</span>{' '}
                        ...
                        {doUtworzenia > 0 && (
                            <div className="mt-4 max-h-48 overflow-auto rounded-xl border border-white/10">
                                {podglad.podglad.map((m, i) => (
                                    ...
                                    >
                                        <span>{m.teamA} <span className="text-white/30">vs</span> {m.teamB}</span>
                                        <span className="font-mono text-white/40">BO{m.bestOf}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="mt-6 flex flex-wrap justify-end gap-3">
                    ...
                    >
                        {doUtworzenia > 0 ? `Utwórz ${doUtworzenia} meczów` : 'Utwórz mecze'}
                    </button>
                </div>
            </div>
        </div>
