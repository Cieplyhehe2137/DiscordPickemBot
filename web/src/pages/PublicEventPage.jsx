...
import {
  ...
  getPublicPrediction,
  getPublicEventPredictions,
  getPublicEventLeaderboard,
  getSwissStats,
  getPublicEventMatchStats,
} from "../lib/api";
...
export default function PublicEventPage() {
  ...
  useEffect(() => {
    ...
    async function loadPublicData() {
      try {
        ...
        try {
          const leaderboardResult = await getPublicEventLeaderboard(slug);
        ...
        try {
          const statsResult = await getSwissStats(slug, "stage1");
  ...
  const communityAccuracy =
    finishedMatches.length > 0
      ? Math.round((communityCorrect / finishedMatches.length) * 100)
      : 0;

  const biggestUpset = finishedMatches
    .filter((match) => match.community_was_right === false)
    .sort((a, b) => {
      const aConfidence = Math.max(a.team_a_percentage, a.team_b_percentage);

      const bConfidence = Math.max(b.team_a_percentage, b.team_b_percentage);
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            <h2 className="text-3xl font-black">Najlepsi gracze</h2>
        ...
        <div className="mt-10 grid gap-6 xl:grid-cols-2">
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            ...
            <h2 className="mt-2 text-3xl font-black">Trendy Swiss - Etap 1</h2>
            ...
            <div className="mt-6 grid gap-4">
              ...
              {(!swissStats ||
                (swissStats?.stats?.three_zero || []).length === 0) && (
                <p className="text-white/50">Brak jeszcze statystyk Swiss.</p>
              )}
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="mt-8 grid grid-cols-1 items-center gap-10 text-center md:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="text-center">
            ...
            <h2 className="mt-2 text-5xl font-black text-violet-300">VS</h2>
        ...
        {selectedMatch.live_status && (
          <p className="mt-3 text-center text-sm font-black uppercase tracking-[0.2em] text-green-300">
            {formatStatusLabel(selectedMatch.live_status)} • MAPA{" "}
            {selectedMatch.current_map || 1}
          </p>
        )}
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          <PublicStat
            title="Faza"
            value={formatPhaseLabel(selectedMatch.phase)}
          />
          <PublicStat title="BO" value={`BO${selectedMatch.best_of || 3}`} />
          <PublicStat
            title="Status"
            value={formatStatusLabel(selectedMatch.ui_status)}
          />
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {["overview", "predictions", "stats"].map((tab) => (
            <button
              ...
              className={`rounded-2xl px-5 py-3 text-sm font-black uppercase tracking-[0.15em] transition ${
                matchModalTab === tab
        ...
        {matchModalTab === "predictions" && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-5">
            ...
            {matchStatsLoading && (
              <p className="mt-3 text-white/50">Ładowanie statystyk typów...</p>
            )}
            ...
            {!matchStatsLoading && (
              <>
                ...
                {(matchStats[selectedMatch.id]?.predictions ?? 0) > 0 && (
                  <div className="mt-6">
                    <div className="flex items-center justify-between text-sm font-bold text-white/50">
                      ...
                      <span
                        className={
                          communityFavorite === "team_b" ? "text-red-300" : ""
                        }
                      >
                ...
                {(matchStats[selectedMatch.id]?.series_breakdown || []).length >
                  0 && (
                  <div className="mt-8">
                    <p className="text-sm uppercase tracking-[0.2em] text-violet-300">
                      Typy na serię
                    ...
                    <div className="mt-4 grid gap-3">
                      {matchStats[selectedMatch.id].series_breakdown.map(
                        (row) => {
                          const total = Number(
                            matchStats[selectedMatch.id]?.predictions || 0,
                          );
                          const percent =
                            total > 0
                              ? Math.round(
                                  (Number(row.picks || 0) / total) * 100,
                                )
                              : 0;
                ...
                {(matchStats[selectedMatch.id]?.map_breakdown || []).length >
                  0 && (
                  <div className="mt-8">
                    <p className="text-sm uppercase tracking-[0.2em] text-violet-300">
            ...
            {publicMatchStats && (
              <>
                <div className="mt-5 grid gap-4 md:grid-cols-3">
                  ...
                  <PublicStat title="Pewność" value={`${confidence}%`} />
                </div>
          ...
          <div className="mt-6 rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
              <p className="text-white/60">
                Dołącz do naszego Discorda i zapisz swój typ, zanim mecz
...
function PredictionModal({ match, closePredictionModal, onPredictionSaved }) {
  ...
  const canSubmit =
    !predictionClosed && isLoggedIn && selectedSeries && mapsValid;
  ...
  async function handleSavePrediction() {
    ...
    try {
      ...
      setError(err?.message || err?.error || "Nie udało się zapisać typu.");
    } finally {
      setSaving(false);
    }
  }
    ...
    >
      ...
      >
            ...
            {seriesOptions.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="mt-2 text-2xl font-black">{option.label}</h3>
                </button>
              );
            })}
          </div>
        ...
        <div className="mt-6 rounded-[2rem] border border-white/10 bg-black/30 p-5">
          ...
          {!selectedSeries && (
            <p className="mt-4 text-white/50">Najpierw wybierz wynik serii.</p>
          )}
          ...
          <div className="mt-6 rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {mapsToPredict.map((mapNo) => (
                ...
                >
                  {getMapLabel(mapNo, match.best_of)}:{" "}
                  {mapScores[mapNo]?.pred_exact_a}:
                  {mapScores[mapNo]?.pred_exact_b}
                </span>
              ))}
            </div>
          </div>
        ...
        {error && (
          <p className="mt-4 text-center font-black text-red-300">{error}</p>
        )}
        ...
        <button
          ...
          className={`mt-8 w-full rounded-2xl px-6 py-4 font-black transition ${
            canSubmit && !saving
