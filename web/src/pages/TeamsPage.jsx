  ...
  async function handleImport() {
    try {
      ...
      setImportError(
        err?.status === 400
          ? err.message
          : describeActionError(err, "zaimportować drużyny"),
      );
    } finally {
  ...
  async function handleSave() {
    try {
      ...
      setFormError(
        err?.status === 409
          ? err.message
          : describeActionError(err, "zapisać drużynę"),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(team) {
    ...
    try {
      ...
      alert(
        err?.status === 409
          ? err.message
          : describeActionError(err, "usunąć drużynę"),
      );
    }
  }
              ...
              {modal !== "add" && (
                <div>
                  ...
                  <p className="mt-2 text-xs text-white/40">
                    Wypełnij tylko wtedy, gdy dostawca nazywa tę drużynę inaczej
                    niż Wy (np. u Was &bdquo;NAVI&rdquo;, u niego &bdquo;Natus
                    Vincere&rdquo;). Panel propozycji wypisze z nazwy drużyny,
      ...
      {modal === "import" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-[2rem] border border-red-500/30 bg-zinc-950 p-8 text-white shadow-2xl">
            ...
            <h2 className="mt-2 text-3xl font-black">Zastąp listę drużyn</h2>

            <p className="mt-2 text-white/50">
              To{" "}
              <strong className="text-red-300">
                usuwa wszystkie {teams.length} istniejących drużyn
              </strong>{" "}
              dla tego serwera i zastępuje je nazwami poniżej. Tej operacji nie
              można cofnąć.
            </p>
            ...
            <div className="mt-6">
              <label className="text-sm font-bold text-white/60">
                Wpisz <span className="text-red-300">REPLACE</span>, aby
                potwierdzić:
              </label>
