...
export default function PublicSwissPickemPage() {
  ...
  const pickedTeams = new Set([...threeZero, ...zeroThree, ...advancing]);
  ...
  return (
    <PageShell>
      ...
      <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-white/50">
              3-0: {threeZero.length}/2 • 0-3: {zeroThree.length}/2 • Awans:{" "}
              {advancing.length}/6
            </p>
          </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <SummaryCard title="Typy 3-0" teams={threeZero} />

        <SummaryCard title="Typy 0-3" teams={zeroThree} />

        <SummaryCard title="Drużyny awansujące" teams={advancing} />
      </div>
      {stats && (
        <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
          ...
          <p className="mt-2 text-white/50">
            Na podstawie {stats.total_predictions} zapisanych typów
            Pick&apos;Em.
          </p>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-3">
            <StatsColumn
              ...
              rows={stats.stats.three_zero}
            />
            <StatsColumn
              title="Najczęściej typowane 0-3"
              rows={stats.stats.zero_three}
            />
            <StatsColumn
              title="Najczęściej typowani awansujący"
              rows={stats.stats.advancing}
            />
          </div>
        </div>
      )}
    ...
    <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
      ...
      <div className="mt-6 grid gap-3">
        {teams.map((team) => {
          ...
          const isPickedElsewhere = pickedTeams.has(team.name) && !isSelected;
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-black/30 p-5">
      ...
      <div className="mt-4 space-y-3">
        ...
        {(rows || []).length === 0 && (
          <p className="text-white/40">Brak jeszcze danych.</p>
        )}
      </div>
    </div>
  );
