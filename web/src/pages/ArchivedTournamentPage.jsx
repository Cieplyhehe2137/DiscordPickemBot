  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        console.error(err);
        if (!cancelled)
          setError(describeActionError(err, "wczytać wyniki turnieju"));
      } finally {
  ...
  const filtered = standings.filter((p) => {
    ...
    return (
      String(p.displayname || "")
        .toLowerCase()
        .includes(q) || String(p.user_id).includes(q)
    );
  });
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {!loading && data && (
          <>
            ...
            <h1 className="mt-3 text-5xl font-black">{event.name}</h1>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              ...
              <Stat
                label="Zwycięzca"
                value={
                  standings[0]?.displayname || standings[0]?.user_id || "—"
                }
              />
            </div>
            ...
            {matches.length === 0 && (
              <p className="mt-6 rounded-2xl border border-yellow-400/20 bg-yellow-500/10 p-4 text-sm text-yellow-200/80">
                Szczegóły meczów tego turnieju zostały usunięte przy jego
                zamykaniu (opcja czyszczenia danych). Klasyfikacja końcowa i
                wyniki faz są zachowane, pełne dane znajdziesz w eksporcie
                Excel.
              </p>
            )}
            ...
            <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <h2 className="text-3xl font-black">Klasyfikacja końcowa</h2>
                    ...
                    >
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-violet-300">
                          {translatePhase(m.phase)} • BO{m.best_of || 3} • #
                          {m.match_no ?? "—"}
                        </p>
...
function PhaseResults({ phaseResults }) {
  ...
  const hasPlayoffs =
    playoffs?.semifinalists?.length || playoffs?.winner?.length;
  const hasPlayin = playin?.teams?.length;
  const hasDouble =
    doubleelim?.upperFinalA?.length || doubleelim?.lowerFinalA?.length;

  if (!swiss.length && !hasPlayoffs && !hasPlayin && !hasDouble) return null;

  return (
    <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
      <h2 className="text-3xl font-black">Oficjalne wyniki faz</h2>
...
function Stat({ label, value }) {
  return (
    <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
      ...
      <h2 className="mt-3 truncate text-3xl font-black">{value}</h2>
    </div>
  );
}
...
function PointsChip({ label, value }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-right">
      ...
      <p className="text-sm font-black text-violet-300">{value}</p>
    </div>
  );
}
