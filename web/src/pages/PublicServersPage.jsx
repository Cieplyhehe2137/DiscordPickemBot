  ...
  useEffect(() => {
    async function loadServers() {
      ...
      try {
        ...
        setLeaderboard((leaderboardResult.leaderboard || []).slice(0, 3));
      } catch (err) {
        console.error(err);
      }
    }
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="animate-fade-in-up relative mt-10 overflow-hidden rounded-[2rem] border border-violet-400/20 bg-violet-500/10 p-8 shadow-[0_0_60px_rgba(139,92,246,0.15)]">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-6 max-w-3xl text-lg text-white/60">
              Dołącz do wydarzeń Pick&apos;Em społeczności, śledź skuteczność
              swoich typów, wspinaj się w rankingach i rywalizuj z innymi fanami
              esportu.
            </p>
              ...
              <div className="mt-6 rounded-[2rem] border border-violet-400/20 bg-black/30 p-6">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-white/40">Połączono z Discord</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            <PublicMiniStat title="Społeczności" value={servers.length} />
        ...
        <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/5 p-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-4xl font-black">Ranking społeczności</h2>
            </div>
        ...
        <div className="mt-10">
          ...
          <p className="mt-4 max-w-2xl text-white/50">
            Przeglądaj aktywne społeczności Pick&apos;Em, dołącz do Discorda i
            śledź wydarzenia na żywo.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {servers.map((server) => (
            ...
            >
              ...
              <h2 className="mt-3 text-4xl font-black">{server.name}</h2>

              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <PublicMiniStat title="Eventy" value={server.events_count} />
                <PublicMiniStat
                  title="Otwarte eventy"
                  value={server.open_events}
                />
              </div>
...
function PublicMiniStat({ title, value }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 p-5">
      ...
      <p className="mt-2 text-3xl font-black text-violet-300">{value}</p>
    </div>
  );
}
