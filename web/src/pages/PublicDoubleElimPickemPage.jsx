    ...
    <PageShell>
      ...
      {!loading && !isLoggedIn && (
        <div className="mt-6 rounded-[2rem] border border-yellow-400/20 bg-yellow-500/10 p-6">
          ...
          <p className="mt-2 text-white/60">
            Zaloguj się przez Discord, aby zapisać swój Pick&apos;Em Double
            Elim.
          </p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-white/50">
              {SECTIONS.map(
                ([key, label]) => `${label}: ${picks[key].length}/2`,
              ).join(" • ")}
            </p>
          </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-4">
        {SECTIONS.map(([key, label]) => (
          <SummaryCard key={key} title={label} teams={picks[key]} />
        ))}
      </div>
    ...
    <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-white/50">Wybierz dokładnie 2 drużyny.</p>
        </div>
