...
test("typy na fazy Swiss maja TRZY segmenty i dlatego sa bezpieczne", async () => {
  // /api/public/events/:slug/swiss-picks nie koliduje z :guildSlug.
  // Przypadek istnieje z tego samego powodu, co przy MVP i wyniku turnieju:
  // przy trasie, ktora wyglada na bezpieczna, najlatwiej przeoczyc
  // pozniejsze skrocenie adresu.
  const tresc = fs.readFileSync(
  ...
  assert.ok(
    tresc.includes('app.get("/api/public/events/:slug/swiss-picks"'),
    "trasa typow na fazy musi miec trzy segmenty po /api/public/",
  );
});
