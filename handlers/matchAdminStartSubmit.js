...
module.exports = async function matchAdminStartSubmit(interaction) {
    try {
        ...
        const [[match]] = await pool.query(
            `SELECT id, team_a, team_b, start_utc_time, is_locked FROM matches WHERE id=? LIMIT 1`,
            [ctx.matchId]
        );
        ...
        const utcDt = parsed.utc;
        const utcJs = utcDt.toJSDate();
        ...
        const utcStr = utcDt.toFormat("yyyy-LL-mm 'UTC'");
