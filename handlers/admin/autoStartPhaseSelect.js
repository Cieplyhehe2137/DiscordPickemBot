...
function buildChannelPage(interaction, eventId, phase, page = 0) {
  ...
  const pageSize = 25;
  const totalPages = Math.max(1, Math.ceil(channels.size / pageSize));
  ...
  const pageChannels = channelArray.slice(start, start + pageSize);
  ...
  const select = new StringSelectMenuBuilder()
    .setCustomId(`auto_start:channel_select:${eventId}:${phase}`)
    .setPlaceholder(`Wybierz kanał — strona ${safePage + 1}/${totalPages}`)
    .addOptions(
...
module.exports = async function autoStartPhaseSelect(interaction) {
  ...
  const payload = buildChannelPage(interaction, eventId, phase, 0);
