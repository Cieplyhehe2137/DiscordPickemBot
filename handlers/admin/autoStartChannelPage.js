...
const { buildChannelPage } = require("./autoStartPhaseSelect");
...
module.exports = async function autoStartChannelPage(interaction) {
  ...
  if (!eventId || !phasesConfig[phase] || !Number.isInteger(page)) {
  ...
  const payload = buildChannelPage(interaction, eventId, phase, page);
