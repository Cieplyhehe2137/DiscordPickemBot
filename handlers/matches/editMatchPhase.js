...
module.exports = async function editMatchPhase(interaction) {
  try {
    if (!interaction.isStringSelectMenu()) {
      return;
    }

    if (!String(interaction.customId || "").startsWith("edit_match_phase:")) {
      return;
    }
    ...
    const phase = String(interaction.values?.[0] || "");
    ...
    if (!matchId || !allowedPhases.has(phase)) {
      return interaction.reply({
        content: "❌ Nieprawidłowe dane meczu.",
        ephemeral: true,
      });
    }
    ...
    return withGuild(
      ...
      async ({ pool, guildId }) => {
        ...
        const modal = new ModalBuilder()
          .setCustomId(`edit_match_modal:${match.id}:${phase}`)
          .setTitle("Edytuj mecz");
        ...
        const startTime = new TextInputBuilder()
          ...
          .setLabel("Start PL: YYYY-MM-DD HH:mm")
          .setStyle(TextInputStyle.Short)
          .setRequired(false)
    ...
    logError("matches", "editMatchPhase failed", {
      message: err.message,
