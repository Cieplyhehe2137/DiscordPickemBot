...
module.exports = async function undoMatchResult(interaction) {
  try {
      ...
      async ({ pool, guildId }) => {
        ...
        if (!matches.length) {
          return interaction.editReply({
            content:
              'ℹ️ Nie ma żadnych rozliczonych meczów do cofnięcia.',
            embeds: [],
            components: []
          });
        }
        ...
        const options = matches.map(match => {
          ...
          return {
            ...
            description:
              `${number} • ${match.phase} • BO${match.best_of}`
                .slice(0, 100),
        ...
        const select =
          new StringSelectMenuBuilder()
            ...
            .setPlaceholder(
              'Wybierz mecz, którego wynik chcesz cofnąć'
            )
            .addOptions(options);
    ...
    if (
      ...
      return interaction.editReply({
        content:
          '❌ Nie udało się pobrać rozliczonych meczów.',
        embeds: [],
        components: []
      }).catch(() => {});
    }
