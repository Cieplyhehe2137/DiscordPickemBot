...
const { isMatchLocked, isMatchStarted } = require("../../utils/matchLock");
  ...
  try {
    ...
    return withGuild(interaction, async ({ pool, guildId }) => {
      ...
      const options = matches.map((match) => {
        const actuallyLocked = isMatchLocked(match);
      ...
      const select = new StringSelectMenuBuilder()
        ...
        .setPlaceholder("Wybierz mecz do zarządzania")
        .addOptions(options);
    ...
    logError("matches", "matchLockManager failed", {
      message: err.message,
      stack: err.stack,
    });
    ...
    return interaction
      .editReply({
        content: "❌ Nie udało się otworzyć zarządzania blokadami.",
        embeds: [],
        components: [],
      })
      .catch(() => {});
